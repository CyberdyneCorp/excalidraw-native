...
/// Validates RoughKit against reference output captured from the real rough.js
/// 4.6.6 (`rough.generator()`), proving exact numeric parity of the seeded RNG
/// and stroke/shape geometry. Reference values were generated with a fixed seed
/// and default options (see scripts/rough_ref.cjs).
final class RoughJSParityTests: XCTestCase {
    private let gen = RoughGenerator()
    private let tol = 1e-4

    private func options(seed: Int) -> RoughOptions {
        var o = RoughOptions()
        o.seed = seed
        return o
    }

    /// Flatten a drawable's ops into `(opName, coordinates)` pairs.
    private func flatten(_ drawable: Drawable) -> [(String, [Double])] {
        drawable.sets.flatMap { set in
            set.ops.map { op -> (String, [Double]) in
                switch op {
                case let .move(p): return ("move", [p.x, p.y])
                case let .lineTo(p): return ("lineTo", [p.x, p.y])
                case let .bcurveTo(c1, c2, e): return ("bcurveTo", [c1.x, c1.y, c2.x, c2.y, e.x, e.y])
                }
            }
        }
    }
    ...
    private func assertMatches(
        _ drawable: Drawable, _ expected: [(String, [Double])], line: UInt = #line
    ) {
        let actual = flatten(drawable)
        XCTAssertEqual(actual.count, expected.count, "op count differs", line: line)
        for (i, (exp, act)) in zip(expected, actual).enumerated() {
            XCTAssertEqual(act.0, exp.0, "op \(i) name", line: line)
            XCTAssertEqual(act.1.count, exp.1.count, "op \(i) arity", line: line)
            for (a, e) in zip(act.1, exp.1) {
                XCTAssertEqual(a, e, accuracy: tol, "op \(i) coord", line: line)
            }
        }
    }

    func testLineMatchesRoughJS() {
    ...
    func testRectangleMatchesRoughJS() {
        let expected: [(String, [Double])] = [
            ("move", [0.857263, 0.963885]),
            ("bcurveTo", [19.680696, 0.960515, 39.440775, -1.932821, 99.206405, 0.392336]),
            ("move", [-0.648666, 0.264599]),
    ...
    func testEllipseMatchesRoughJS() {
        let expected: [(String, [Double])] = [
...
extension RoughJSParityTests {
    /// Hachure fill is visually faithful but not line-identical to rough.js: our
